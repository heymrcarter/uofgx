...
export function getPendingMembers({ commit, state, rootState }) {
  return new Promise((resolve, reject) => {
    clanService
      .getPendingMembers(process.env.CLAN_ID, rootState.session.access_token)
      .then(response => {
...
export function approvePendingMemberships({ commit, state, rootState }, membershipIds) {
  return new Promise((resolve, reject) => {
    clanService
      .approveMembershipRequest(process.env.CLAN_ID, membershipIds, rootState.session.access_token)
      .then(response => {
